...
impl System {
    ...
    pub fn condition(&self, cc: ConditionCode) -> bool {
        ...
        match cc {
            ...
            ConditionCode::LE => (flags & (1 << 3) != 0) != (flags & (1 << 2) != 0) || flags & (1 << 0) != 0,
            ...
            ConditionCode::G => !((flags & (1 << 3) != 0) != (flags & (1 << 2) != 0) || flags & (1 << 0) != 0),
    ...
    pub fn alu_binaryb(&mut self, op: AluBinaryOperation, lhs: u8, rhs: u8) -> u8 {
        ...
        let (value, cf, of) = match op {
            ...
            Sar => {
                let shift = u32::from(rhs & 7);
                let cf = shift != 0 && lhs & (1 << (shift - 1)) != 0;
                (lhs.cast_signed().wrapping_shr(shift).cast_unsigned(), cf, false)
            },
            Rol => {
                let shift = u32::from(rhs & 7);
                let cf = shift != 0 && lhs & (0x80 >> (shift - 1)) != 0;
        ...
        let zf = value == 0;
        self.set_regw(FL, (flags & !15) | (u16::from(zf) << 0) | (u16::from(cf) << 1) | (u16::from(sf) << 2) | (u16::from(of) << 3));
        value
    }
    ...
    pub fn alu_unaryb(&mut self, op: AluUnaryOperation, value: u8) -> u8 {
        ...
        let (result, cf, of) = match op {
            ...
            Swap => {
                (value.rotate_right(4), false, false)
            },
            Popcnt => {
                (value.count_ones() as u8, false, false)
            },
            Rcl => {
                let cf = value & 0x80 != 0;
            ...
            Rcr => {
                let cf = value & 1 != 0;
                (value.wrapping_shr(1) | (u8::from(flags & 2 != 0) << 7), cf, false)
            },
            Zero => {
                (value, value == u8::MAX, value == i8::MAX.cast_unsigned())
            },
        };
    ...
    pub fn alu_unaryw(&mut self, op: AluUnaryOperation, value: u16) -> u16 {
        ...
        let (result, cf, of) = match op {
            ...
            Popcnt => {
                (value.count_ones() as u16, false, false)
            },
            Rcl => {
            ...
            Rcr => {
                let cf = value & 1 != 0;
                (value.wrapping_shr(1) | (u16::from(flags & 2 != 0) << 15), cf, false)
            },
            Zero => {
                (value, value == u16::MAX, value == i16::MAX.cast_unsigned())
            },
        };
        let sf = result.cast_signed() < 0;
        let zf = result == 0;
...
impl System {
    pub fn step(&mut self) -> Result<Opcode, Box<dyn Error>> {
        ...
        let bytes= (0..4).map(|x| self.get_memb(self.get_regw(WordRegister::PC).wrapping_add(x))).collect::<Result<Vec<u8>, _>>()?;
        let opcode = Opcode::from_slice(&bytes)?;

        self.reg_pc += opcode.to_vec().len() as u16;
        match opcode {
            MovRIB(dst, imm) => {
            ...
            MovCcRRB(cc, dst, src) => {
                if self.condition(cc) {self.set_regb(dst, self.get_regb(src))};
            }
            MovCcRRW(cc, dst, src) => {
                if self.condition(cc) {self.set_regw(dst, self.get_regw(src))};
            }
            XchCcRRB(cc, dst, src) => {
            ...
            MovCcROB(cc, dst, offset, base) => {
                if self.condition(cc) {self.set_regb(dst, self.get_memb(self.get_regw(base).wrapping_add_signed(offset.into()))?)};
            }
            MovCcROW(cc, dst, offset, base) => {
                if self.condition(cc) {self.set_regw(dst, self.get_memw(self.get_regw(base).wrapping_add_signed(offset.into()))?)};
            }
            LeaCcROB(cc, dst, offset, base) => {
                if self.condition(cc) {self.set_regb(dst, self.get_regw(base).wrapping_add_signed(offset.into()) as u8)};
            }
            LeaCcROW(cc, dst, offset, base) => {
                if self.condition(cc) {self.set_regw(dst, self.get_regw(base).wrapping_add_signed(offset.into()))};
            }

            JmpCcA(cc, addr) => {
                if self.condition(cc) {self.set_regw(PC, addr)};
            }
            CallCcA(cc, addr) => {
                if self.condition(cc) {
            ...
            MovCcORB(cc, offset, base, src) => {
                if self.condition(cc) {self.set_memb(self.get_regw(base).wrapping_add_signed(offset.into()), self.get_regb(src))?};
            }
            MovCcORW(cc, offset, base, src) => {
                if self.condition(cc) {self.set_memw(self.get_regw(base).wrapping_add_signed(offset.into()), self.get_regw(src))?};
            }
            ...
            JrCcX(cc, offset) => {
                if self.condition(cc) {self.set_regw(PC, self.get_regw(PC).wrapping_add_signed(offset.into()));}
            },
            ...
            ClbRRW(dst, bitreg) => {
                self.set_regw(dst, self.get_regw(dst) & !(1 << (self.get_regb(bitreg) & 15)))
            },
            ...
            TbitRIB(src, bit) => {
                self.set_regw(FL, (self.get_regw(FL) & !1) | if self.get_regb(src) & (1 << (bit & 7)) != 0 {1} else {0});
            },
            TbitRIW(src, bit) => {
                self.set_regw(FL, (self.get_regw(FL) & !1) | if self.get_regw(src) & (1 << (bit & 15)) != 0 {1} else {0});
            },
            TbitRRB(src, bitreg) => {
                self.set_regw(FL, (self.get_regw(FL) & !1) | if self.get_regb(src) & (1 << (self.get_regb(bitreg) & 7)) != 0 {1} else {0});
            },
            TbitRRW(src, bitreg) => {
                self.set_regw(FL, (self.get_regw(FL) & !1) | if self.get_regw(src) & (1 << (self.get_regb(bitreg) & 7)) != 0 {1} else {0});
            },

            Nop => {},
            Halt => {self.set_regw(FL, self.get_regw(FL) | (1 << 15));},
        };
        Ok(opcode)
    }
}